...
# ─────────────────────────────────────────────────────────────────────────────
# Adaptive CD-TS (Step 3): drift tracking WITHOUT a known variation budget V_T.
#
# theory_gamma(T, V_T) tunes the discount from an oracle that knows V_T a
# priori — unrealistic in production. The adaptive variant drops that
# assumption: each arm carries a two-sided CUSUM change detector over its own
# reward stream (Cao, Wen, Kveton & Xie, AISTATS 2019, "Nearly Optimal Adaptive
# Procedure with Change Detection for Piecewise-Stationary Bandit"; the CUSUM
# statistic is the Page–Hinkley test, Page 1954 / Hinkley 1971). On a flagged
# change the arm's Beta posterior is restarted, discarding stale pre-change
# evidence. Detector thresholds (delta, lam) are fixed constants that do NOT
# depend on V_T or T — that is precisely what makes the policy assumption-free.
# ─────────────────────────────────────────────────────────────────────────────


class CUSUMDetector:
    """Two-sided CUSUM change detector on a scalar reward stream.

    Design follows Cao et al. (2019): a `warmup` window estimates a stable
    reference mean μ₀, after which two non-negative cumulative sums accumulate
    deviations above/below μ₀ by more than the slack `delta`. An alarm fires
    when either sum exceeds the threshold `lam`. It needs no prior knowledge of
    the change magnitude or frequency (no V_T).

    Defaults are tuned for {0,1}-bounded (e.g. Bernoulli) rewards to be
    *conservative*: average run length to a false alarm is on the order of 10⁵
    rounds, while a large abrupt shift (≳0.4 in mean) is caught within a few
    dozen rounds. Gentle/gradual drift is intentionally left to the geometric
    discount in `AdaptiveCDTSPolicy`; the detector exists for abrupt breaks
    (model deprecation, outage) where forgetting-by-discount is too slow.

    A fixed reference makes the statistic blind once the post-change regime
    dominates the warmup estimate, so callers must `reset()` after acting on an
    alarm to re-baseline on the new regime.
    """

    def __init__(self, warmup: int = 50, delta: float = 0.2, lam: float = 12.0):
        self.warmup = warmup
        self.delta = delta
        self.lam = lam
        self.reset()
    ...
    def reset(self) -> None:
        self.n = 0
        self.ref = 0.0    # μ₀ estimated over the warmup window
        self.g_pos = 0.0  # accumulates upward shifts
        self.g_neg = 0.0  # accumulates downward shifts

    def update(self, x: float) -> bool:
        """Feed one observation; return True iff a change is detected."""
        self.n += 1
        if self.n <= self.warmup:
            self.ref += (x - self.ref) / self.n
            return False
        dev = x - self.ref
        self.g_pos = max(0.0, self.g_pos + dev - self.delta)
        self.g_neg = max(0.0, self.g_neg - dev - self.delta)
        return self.g_pos > self.lam or self.g_neg > self.lam


class AdaptiveCDTSPolicy(Policy):
    """CD-TS that tracks drift with per-arm change detection instead of an
    oracle-tuned discount. Never sees V_T or T.

    Between detections it applies a mild geometric discount (slow-drift safety
    net); on a CUSUM alarm for the played arm it restarts that arm's posterior
    to the uniform prior. `n_restarts` is exposed for diagnostics.
    """
    name = "adaptive_cd_ts"
    ...
    def __init__(
        ...
        self.arms = [ArmStats() for _ in range(K)]
        self.detectors = [CUSUMDetector(delta=delta, lam=lam) for _ in range(K)]
        self.gamma = gamma
        self.floor = floor
        self.n_restarts = 0
    ...
    def update(self, arm: int, reward: float) -> None:
        if self.detectors[arm].update(reward):
            # Drift on this arm: discard stale evidence and restart.
...
def run_single_horizon(
    ...
    for seed in range(n_seeds):
        ...
        policies = [
            ...
            # Adaptive CD-TS gets NO V_T-tuned gamma — it detects drift online.
            AdaptiveCDTSPolicy(K),
